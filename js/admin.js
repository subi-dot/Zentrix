...
// We will use Firestore for products, users, and orders.
// For cart and current user sessions, we can keep using localStorage/sessionStorage 
// as they are device-specific and shouldn't heavily hit the DB until checkout/login.
...
if(productForm) {
    productForm.addEventListener('submit', (e) => {
        ...
        const prodData = { name, category, price: Number(price), image, designer, customizationPrompt };
        
        let promise;
        if (idVal) {
            // Edit
            prodData.id = Number(idVal); // Keep ID consistency if it's numeric
            promise = db.collection('products').doc(idVal.toString()).set(prodData, {merge: true});
        } else {
            // Add
            const nextId = adminProducts.length > 0 ? Math.max(...adminProducts.map(p=>Number(p.id) || 0)) + 1 : 1;
            prodData.id = nextId;
            promise = db.collection('products').doc(nextId.toString()).set(prodData);
        }

        promise.then(() => {
            productModal.classList.remove('active');
            // renderInventory() will auto-fire due to onSnapshot
        }).catch(err => {
            console.error("Error saving product:", err);
            alert("Matrix Error: Failed to save blueprint.");
        });
    });
}
...
window.deleteProduct = function(id) {
    if(confirm("Confirm destructive action on catalog item?")) {
        db.collection('products').doc(id.toString()).delete().catch(err => {
...
db.collection('orders').get().then(snapshot => {
    if (snapshot.empty) {
        defaultOrders.forEach(o => {
            db.collection('orders').doc(o.id.toString()).set(o);
        });
    }
});
db.collection('users').get().then(snapshot => {
    if (snapshot.empty) {
        defaultUsers.forEach(u => {
            db.collection('users').doc(u.id.toString()).set(u);
        });
    }
});

// Render Orders with real-time listener
...
function renderOrders() {
    ...
    unsubscribeOrders = db.collection('orders').orderBy('date', 'desc').onSnapshot((snapshot) => {
        ordersBody.innerHTML = '';
        snapshot.forEach(doc => {
            const ord = doc.data();
            // ensure id is set in data or use doc.id
        ...
        console.error("Firestore Order Stream Error:", err);
    });
}
...
window.deleteUser = function(id) {
    if(confirm("Confirm termination of user nexus link (Delete User)?")) {
        db.collection('users').doc(id.toString()).delete().then(() => {
            renderUsers(); // Manually refresh since it's a static get
        }).catch(err => {
            console.error("Delete Error:", err);
            alert("Delete Protocol Failed.");
        });
...
if (saveOrderStatusBtn) {
    saveOrderStatusBtn.addEventListener('click', () => {
        db.collection('orders').doc(currentOrderId.value).update({
            ...
            alert("System Error: Could not sync status to cloud.");
        });
    });
}
