...
// Initialize Data if empty (Sync defaults to Firestore if products collection is empty)
db.collection('products').get().then(snapshot => {
    if (snapshot.empty) {
        console.log("Initializing default products to Firestore...");
        defaultProducts.forEach(prod => {
            db.collection('products').doc(prod.id.toString()).set(prod);
        });
    }
...
function renderUserOrders() {
    ...
    unsubscribeUserOrders = db.collection('orders')
    ...
    `;
    }, err => console.error("User orders stream error:", err));
}
...
if(signupForm) {
    signupForm.addEventListener('submit', (e) => {
        ...
        // Check unique email in Firestore
        ...
        emailjs.send('service_6bhccr2', 'template_kn3dcuq', templateParams)
        ...
        // Close out the DB query scope
            ...
            alert("Matrix Error: User lookup failed.");
        });
    });
}
...
if(verifyForm) {
    verifyForm.addEventListener('submit', (e) => {
        ...
        if(codeInput === verificationCode) {
            // Success
            db.collection('users').doc(pendingVerificationUser.id).set(pendingVerificationUser)
                .then(() => {
...
function renderProducts(filter = 'all') {
    ...
    query.get().then((snapshot) => {
        ...
        // Save to global products array for cart quick access
        products = filtered;
    ...
    filtered.forEach(prod => {
        ...
        productGrid.innerHTML = `<p class="neon-text text-center w-100" style="grid-column: 1/-1;">Matrix disconnection: Failed to load blueprints.</p>`;
    });
}
...
function renderFullUserOrders() {
    ...
    db.collection('orders').where('customer', '==', currentUser.email).get().then(snapshot => {
        const myOrders = [];
        snapshot.forEach(doc => {
            const o = doc.data();
            o.id = o.id || doc.id;
    ...
    if(confirm("Are you certain you wish to abort this transmission? (Cancel Order)")) {
        db.collection('orders').doc(id).get().then(doc => {
            if (doc.exists && doc.data().status === 'Processing') {
                return db.collection('orders').doc(id).update({ status: 'Cancelled' });
            } else {
                throw new Error("Order cannot be aborted at its current status.");
            }
        }).then(() => {
            renderFullUserOrders();
            // renderUserOrders works via snapshot automatically!
